...
;     by the IDL. It may prevent the code rasing errors. If the code doesn't operate well
;     but raise no error, turn off 'CATCH' procedure, and commnet out related lines.
; HISTORY
;     Geographic -> Geomagnetic conversion added. Jul/09/2018
;     Rotating, clipping, and flipping procedures are replaced by 2D-interpolation. Feb/16/2019
;     Handling images with 10S and 20S exporsure time is added. Mar/01/2021
;     
;     Meaningless suffix of the old image is converted meaningful suffix containing
;     exposure-time information for the image-acquision ('05S') Mar/03/2021
;-
pro Process_JBS_AASC_raw_images_rev2
...
target_yr=2021
  ...
  in_dir='/Volumes/Backup Plus/aasc/2021_raw/'
  out_dir='/Volumes/Backup Plus/Processed/'
  file_mkdir, out_dir
...
;  out_dir=strjoin(strsplit(out_dir,'/\',/ext),path_sep())+path_sep()
  ...
  sjday=julday(03,25,target_yr,0)
  ;cdf_epoch,sday,target_yr,03,01,/compute
  ;sjday=sday/86400000
  ejday=julday(12,31,target_yr,0)
  ;cdf_epoch,eday,target_yr,12,31,/compute
  ;ejday=eday/86400000
  ...
  for jd=0L,num_day-1 do begin
    caldat,jday0,cmon,cday,cyr,chr
  ;  cdf_epoch,jday0,cyr,cmon,cday,chr,/break
...
;      string(cday,format='(i2.2)')+path_sep()+'*'+path_sep()+'*.png',count=num_file)
    for chr=chr, 23 do begin    
    fname=file_search('/Volumes/Backup Plus/aasc/2021_raw/'+$
         ; string(cyr,format='(i4)')+'/'+string(cmon,format='(i2.2)')+'/'+string(cday,format='(i2.2)')+'/'+string(chr,format='(i2.2)')+'/*.png',count=num_file)
         ;  string(cmon,format='(i2.2)')+'/'+string(cday,format='(i2.2)')+'/'+string(chr,format='(i2.2)')+'/*.png',count=num_file)
           string(cmon,format='(i2.2)')+'/'+string(cday,format='(i2.2)')+'/*/*.png',count=num_file)
           stop 
    ...
    if num_file eq 1 then begin
      ...
      ccursor=0L
      while 1 do begin
        if n_elements(spl_fbname[ccursor]) ne num_underbar+2 then begin
      ...
      goto, no_image
    endelse
;; This part makes error!! Ignore it!
;    sor=sort(UT)
;    UT=UT[sor]
...
;;
    ...
    for i=0L,num_file-1 do begin
     ...
     ; ff = strsplit(fname[i], '_', /ext)
     ; if (ff[-1] ne '20S.png') then begin
      if (suffix[i] ne '20S') then begin
     ; stop 
      catch,err_stat
      if err_stat ne 0 then goto,skip_img
;; This part must be updated because the suffix has become important since 06/03/2020
...
;;
     ...
     ; tv,bytscl(corr_img,max=img_max_inten,min=img_min_inten),/true
      ...
      ;    loadct,0,/sil
;      if suffix[i] eq '1' or suffix[i] eq '2' then begin
;        tail='_05S'
;      endif else if suffix[i] eq '10S' or suffix[i] eq '20S' then begin
;        tail='_'+suffix[i]
;      endif else begin
;        tail='_'+suffix[i]
;      endelse
    
       if suffix[i] eq '10S' then begin
        tail='_'+suffix[i]
       endif else if suffix[i] ne '10S' then continue

      write_png,out_dir+sub_dir+'JBS_Aurora_ASC_'+dstring+'_'+HHMMSS[i]+tail+'.png',corr_img
